                ...
                for j in range(1, len(self.matrix)):
                    for k in range(len(self.matrix)):
                        ...
                        if k == i:
                            ...
                            self.subMatrix.matrix[j -
                                                  1].append(self.matrix[j][k])
...
A = Matrix([[1, 2, 3, 4, 5], [23, 4, 32, 12, 18], [98, -6, -8,
           90, 10], [11, 87, 76, 3, 9], [-11, 2, 33, 4, 12]])
B = Matrix([[1, 88, 3, 7, 5], [23, 4, 32, 12, 18], [
           98, -6, -8, 90, 10], [11, 87, 76, 3, 9], [-11, 2, 33, 4, 12]])


def multiply(A: Matrix, B: Matrix):
    product_matrix = Matrix([[sum(list(map(lambda x, y:x*y, r, c)))
                              for c in B.transpose().matrix] for r in A.matrix])
    for row in product_matrix.matrix:
        for column in row:
            print(column)
        print()
